<template>
       ...
       <div class="row">
           <div class="col-md-12">
               <div class="panel panel-default">
                ...
                <button @click="inserttosala()" class="btn btn-success " style="padding:5px">
                     show sala product
                     </button>
                   </div>
                   ...
                   <div class="modal-body">
                       ...
                       <div class="form-group">
                           ...
                           <textarea name="description" id="description" cols="30" rows="5" class="form-control"
                                     placeholder="Product Description" v-model="task.description"></textarea>
                       </div>
                   </div>
           ...
           <div class="modal-dialog" role="document">
               <div class="modal-content">
                   <div class="modal-header">
                       <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                               aria-hidden="true">&times;</span></button>
                       <h4 class="modal-title">add Quantity Product</h4>
                   </div>
                   <div class="modal-body">
                       <div class="alert alert-danger" v-if="errors.length > 0">
                           <ul>
                       ...
                       <div class="form-group">
                           <label>Name:</label>
                           <input type="text" placeholder="Task Name" class="form-control" readonly
                                  v-model="update_task.name">
                       </div>
                       <div class="form-group">
                           <label for="description">Description:</label>
                           <textarea readonly cols="30" rows="5" class="form-control"
 ...
 <div class="modal fade" tabindex="-1" role="dialog" id="show_sala_model">
           <div class="modal-dialog" role="document">
               <div class="modal-content">
                   <div class="modal-header">
                       <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                               aria-hidden="true">&times;</span></button>
                       <h4 class="modal-title">show sala</h4>
                   </div>
                   <div class="modal-body">
                         <table class="table table-bordered table-striped table-responsive" v-if="tasks.length > 0">
                           <tbody>
...
<script>
   export default {
       ...
       methods: {
           ...
           inserttosala()
            {
        $("#show_sala_model").modal("show");
               $.ajax({
            url: 'task/inserttosala',
            type: 'get',
            data: {
...
if (error.response.data.errors && error.response.data.errors.description)
           ...
           updateTask()
           {
               axios.patch('/task/' + this.update_task.id, {
                   ...
                   Quantity:this.update_task.Quantity,
               })
                   .then(response => {
